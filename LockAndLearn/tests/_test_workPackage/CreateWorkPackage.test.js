...
describe('CreateWorkPackage', () => {
    it('renders correctly', () => {
      ...
      it('updates subcategories based on selected subject and grade', async () => {
        const { getByTestId, findByTestId } = render(<CreateWorkPackage />);
        ...
        // Wait for the subcategory picker to update
        const subcategoryPicker = await findByTestId('subcategory-picker');
      
        // Ensure that subcategoryPicker is not undefined
        if (subcategoryPicker && subcategoryPicker.props && subcategoryPicker.props.children) {
          expect(subcategoryPicker.props.children).toContain('Numbers');
          expect(subcategoryPicker.props.children).toContain('Substraction');
        } else {
          // Cause the test to fail if subcategoryPicker is not as expected
      ...
      it('creates a work package and navigates on successful response', async () => {
        ...
        await waitFor(() => {
          ...
          expect(mockNavigate).toHaveBeenCalledWith('DisplayWorkPackageContent', { workPackageId: 'wp123' });
        });
      });

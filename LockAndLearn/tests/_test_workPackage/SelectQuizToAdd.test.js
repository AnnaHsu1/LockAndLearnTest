...
global.fetch = jest.fn(() =>
    Promise.resolve({
        json: () => Promise.resolve([{ _id: '1', name: 'Quiz 1' }, { _id: '2', name: 'Quiz 2' }]),
    })
);
...
describe('SelectQuizToAdd', () => {
    ...
    it('conditionally renders the add button', async () => {
        ...
        if (addButton) {
            ...
            expect(useNavigation().navigate).toHaveBeenCalledWith(/* your expected navigation arguments */);
    ...
    it('deletes a quiz and updates state on successful response', async () => {
        ...
        if (deleteButton) {
            ...
            // Assertions
            await waitFor(() => {
                expect(fetch).toHaveBeenCalledWith('http://localhost:4000/quizzes/deleteQuiz/1', { method: 'DELETE' });
